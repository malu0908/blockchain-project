...
module.exports.info  = 'Reading information from file d_labitems';
...
function processFile(){
    ...
    const csvString = fs.readFileSync("/home/ana/physionet.org/files/mimiciii/1.4/D_LABITEMS.csv").toString("utf-8");
...
function generateWorkload() {
    ...
    for(let i= total-1; i < total; i++) {
        ...
        if (bc.getType() === 'fabric') {
            workload.push({
                chaincodeFunction: 'insertDlabitem',
                chaincodeArguments: [csv[total][0], csv[total][1], csv[total][2], 
                csv[total][3], csv[total][4], csv[total][5]]
            });
        } 
    }
    return workload;
